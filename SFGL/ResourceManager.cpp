...
std::string getMimeType(const std::string& extension) {
    ...
    if (RegOpenKeyEx(HKEY_CLASSES_ROOT, extension.c_str(), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        // get content type
        if (DWORD dwBuffSize = sizeof(szBuffer);
            RegQueryValueEx(
            hKey, "Content Type", nullptr, nullptr, (LPBYTE) szBuffer, &dwBuffSize) ==
            ERROR_SUCCESS) {
            // success
            result = szBuffer;
        }
...
bool ResourceManager::load(const std::string& key, const std::string& file) {
    ...
    std::string type =
    getMimeType(file.substr(file.find_last_of('.'), file.size() - file.find_last_of('.')));
    ...
    // If the file is a json, load it as a json
        ...
        if (jFile.is_open()) {
            ...
            std::cout << "Failed to load json " << file << ". Reason: Unable to open file"
                      << std::endl;
        }
    }
    // If the file is an audio file
        ...
        if (size < 0) {
            std::cout << "Failed to load audio " << file << ". Reason: Unable to open file"
                      << std::endl;
        }
...
void ResourceManager::setAudioLimit(unsigned long long limit) { audioLimit = limit; }

unsigned long long ResourceManager::getAudioLimit() const { return audioLimit; }
