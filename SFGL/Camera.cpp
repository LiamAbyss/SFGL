...
sf::RenderWindow& Camera::getWindow() { return static_cast<sf::RenderWindow&>(*window); }

void Camera::setWindow(sf::RenderWindow& w) { window = &w; }
...
void Camera::update(sf::Time dt) {
    if (time.asSeconds() > 0) {
        ...
        float shakeSizeX = (rand() % 2 ? -1.F : 1.F) *
                           static_cast<float>(rand() % static_cast<int>(ceil(intensity / 2))) +
                           intensity;
        float shakeSizeY = (rand() % 2 ? -1.F : 1.F) *
                           static_cast<float>(rand() % static_cast<int>(ceil(intensity / 2))) +
                           intensity;
        float shakeSizeY2 = (rand() % 2 ? -1.F : 1.F) *
                            static_cast<float>(rand() % static_cast<int>(ceil(intensity / 2))) +
                            intensity;
